...
class PathologistListPage extends StatefulWidget {
  const PathologistListPage({super.key});

  @override
  State<PathologistListPage> createState() => _PathologistListPageState();
}

class _PathologistListPageState extends State<PathologistListPage> {
  ...
  List<PathologistModel> pathologistModelList=[];
  List<PathologistModel> filterPathologistModel = [];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: IAppBar.commonAppBar(title: "pathology_lab".tr),
  ...
  _buildBody() {
    return ListView(
      ...
      children: [
        ...
        filterPathologistModel.isEmpty?
        const NoDataWidget():
        _buildClinic(filterPathologistModel)
      ],
    );
  }
  void filterClinics(String query) {
    if (query.isEmpty) {
      setState(() {
        filterPathologistModel = pathologistModelList;
      });
    } else {
      setState(() {
        filterPathologistModel = pathologistModelList.where((doc) {
  ...
  _buildClinicCard(PathologistModel pathologistModel){
    return Card(
      ...
      child: ListTile(
        onTap: (){
          Get.toNamed(RouteHelper.getPathologyPageRoute(pathId: pathologistModel.id.toString()));
        },
        title: Text(pathologistModel.title??"",
          style: TextStyle(
  ...
  void getAndSetData() async{
    ...
    final res=await PathologistService.getData(start: "",end:"",cityId: selectedCityId);
    if(res!=null){
      pathologistModelList=res;
      filterPathologistModel.addAll(pathologistModelList);
    }
    setState(() {
      _isLoading=false;
    });
  ...
  getClinicData()async{
    ...
    pathologistModelList.clear();
    filterPathologistModel.clear();
